...
class GhostDaemon:
    ...
    def process_inbound(self, msg):
        ...
        system_prompt = (
            ...
            "- Run until FULLY COMPLETE. NEVER tell the user to do it themselves.\n"
            "- When DONE, call `task_complete(summary='...')`. No plain text endings.\n"
            "- NEVER hallucinate. Only state VERIFIED facts.\n"
            "- If a tool fails, escalate — try a different tool, write a script, combine approaches. "
            "Keep going until the task is done.\n\n"
            "### ESCALATION LADDER (follow this when built-in tools fail):\n"
            "**Level 1 — Built-in tools**: web_fetch, browser, web_search, shell_exec, etc.\n"
            "**Level 2 — Python sandbox**: Write a script in `/tmp/ghost_sandbox/` with its own venv:\n"
            ...
            "  2. Write script to `/tmp/ghost_sandbox/run.py` via file_write\n"
            "  3. `shell_exec(command='cd /tmp/ghost_sandbox && source .venv/bin/activate && python3 run.py')`\n"
            "  NEVER modify Ghost source code or Ghost's `.venv`. The sandbox isolates everything.\n"
            "**Level 3 — Browser automation**: Navigate, click, extract via the browser tool.\n"
            "**Level 4 — Combine**: Chain tools (e.g. browser for auth + Python for data processing).\n\n"
            "Once you solve it, if the solution is reusable, submit as a permanent Ghost tool:\n"
    ...
    def check_actions(self):
        ...
        if action_id == "ask" and source:
            ...
            system_prompt = (
                ...
                "- You NEVER give up. You NEVER tell the user to do it themselves.\n"
                ...
                "### ESCALATION LADDER (follow this when a tool or approach fails):\n"
                "**Level 1 — Built-in tools**: web_fetch, browser, web_search, shell_exec, grep, etc.\n"
                "**Level 2 — Python sandbox**: Write a script in `/tmp/ghost_sandbox/` with its own venv:\n"
                "  1. `shell_exec(command='mkdir -p /tmp/ghost_sandbox && cd /tmp/ghost_sandbox && "
                "python3 -m venv .venv && source .venv/bin/activate && pip install <pkg>')`\n"
                "  2. Write script to `/tmp/ghost_sandbox/run.py` via file_write\n"
