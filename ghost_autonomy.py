...
GROWTH_ROUTINES = [
    ...
    },
    {
        ...
        "prompt": (
            ...
            "   ⚠️ If the output contains a 'PAST PR REJECTIONS' section, READ EVERY SINGLE ONE.\n"
            "   These are reviewer feedback from previous failed attempts. Your code MUST address\n"
            "   ALL listed issues — missing even one will get the PR rejected again.\n"
            ...
            "   b) STUDY AN EXISTING SIMILAR FEATURE (most important step):\n"
            "      - Find a ghost_*.py module that does something similar to what you're building.\n"
            "        E.g. building file logging? Read ghost_memory.py for I/O patterns.\n"
            "        Building a new dashboard page? Read an existing routes/*.py + pages/*.js pair.\n"
            "      - file_read that module. Note how it handles: file I/O, locking, error handling,\n"
            "        tool definitions, config integration. FOLLOW THE SAME PATTERNS.\n"
            "      - If your feature involves file writes, grep('os.replace', include='*.py') to see\n"
            "        how existing code does atomic writes. Copy that exact pattern.\n"
            "      - Do NOT invent new patterns when working patterns already exist in the codebase.\n"
            "   c) grep('relevant_pattern', include='*.py') to find existing related code.\n"
            "   d) file_read on the files you'll modify — understand the FULL context.\n"
            "   e) glob('ghost_*.py') to see all existing modules and avoid conflicts.\n"
            ...
            "   e) Run through the PRE-PR SELF-REVIEW checklist (see below). Every item must pass.\n"
            ...
            "    ⚠️ IF THE PR IS REJECTED: call task_complete IMMEDIATELY. Do NOT retry in this session.\n"
            "    The system automatically: (a) accumulates ALL reviewer feedback in pr_rejections[],\n"
            "    (b) enforces a 15-minute cooldown, then (c) re-fires the Evolution Runner with\n"
            "    full context of every past rejection. Each retry attempt gets smarter.\n"
            "    Retrying in the same session wastes tokens and produces the same bugs.\n"
            "    Do NOT call fail_future_feature — retry scheduling is fully automatic.\n"
            ...
            "## COMPLETENESS RULES (MOST COMMON REASON FOR PR REJECTION)\n"
            "These are the EXACT rules the PR reviewer uses. Violating ANY = instant rejection.\n\n"
            "### Frontend-Backend Integration (caused M-14, M-15, M-23 — the most damaging bugs)\n"
            "- Backend API added = frontend JS MUST call it. Dead endpoints = BLOCKED.\n"
            "- Frontend UI added = backend MUST persist and return data.\n"
            "- If the feature description mentions UI (autocomplete, dropdown, toggle, page),\n"
            "  you MUST modify the JS file. No exceptions.\n"
            "- Count your changed files: if you have routes/<x>.py but NO pages/<x>.js,\n"
            "  your PR is INCOMPLETE and will be rejected.\n\n"
            ...
            "- If any of these are missing, the feature is dead code — BLOCKED.\n\n"
            "### Thread Safety and File I/O (caused repeated PR rejections)\n"
            "- Shared files need locking or atomic writes (tempfile + os.replace)\n"
            "- Write to new paths = Path.mkdir(parents=True, exist_ok=True) FIRST\n"
            "- Never read an entire unbounded file into memory — use limits\n"
            "- No read-modify-write without a threading.Lock()\n\n"
