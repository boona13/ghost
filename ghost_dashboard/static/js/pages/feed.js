...
export async function render(container) {
  ...
  container.innerHTML = `
    <div class="flex items-center justify-between mb-1">
      ...
      <button id="btn-refresh-feed" class="btn btn-ghost btn-sm">Refresh</button>
    </div>
    <p class="page-desc">Recent Ghost actions — auto-refreshes every 5 seconds</p>

    <div class="flex flex-wrap gap-2 mb-4">
      <button class="badge badge-purple cursor-pointer feed-filter active" data-filter="all">All (${entries.length})</button>
      ${types.map(t => {
        const c = entries.filter(e => e.type === t).length;
        return `<button class="badge badge-${u.TYPE_COLORS[t] || 'zinc'} cursor-pointer feed-filter" data-filter="${t}">${u.TYPE_ICONS[t] || ''} ${t} (${c})</button>`;
      }).join('')}
    </div>
...
function renderFeed(entries, u) {
  if (!entries.length) return '<div class="text-sm text-zinc-600">No activity</div>';
  return entries.map(e => `
    <div class="feed-entry type-${e.type || 'unknown'}">
      <div class="flex items-center gap-2 mb-2">
        ...
        ${e.tools_used?.length ? `<span class="text-[10px] text-zinc-600">${e.tools_used.length} tools</span>` : ''}
