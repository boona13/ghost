...
def _process_message(session, daemon):
    ...
    try:
        ...
        system_prompt = (
            ...
            "If a tool fails, you escalate. If that fails, you escalate again. You keep going until "
            "the task is done. Follow this escalation ladder:\n\n"
            "**Level 1 — Built-in tools**: web_fetch, browser, web_search, shell_exec, etc.\n"
            "**Level 2 — Quick Python script**: If built-in tools can't handle it, write a Python "
            "script in a temp sandbox. Steps:\n"
            "  1. `shell_exec(command='mkdir -p /tmp/ghost_sandbox && cd /tmp/ghost_sandbox && "
            "python3 -m venv .venv && source .venv/bin/activate && pip install <needed-pkg>')`\n"
            "  2. Write your script to `/tmp/ghost_sandbox/run.py` via file_write\n"
            "  3. `shell_exec(command='cd /tmp/ghost_sandbox && source .venv/bin/activate && python3 run.py')`\n"
            "  4. Read and return the output to the user\n"
            "  IMPORTANT: Use `/tmp/ghost_sandbox/` — NEVER modify Ghost's own source code or project venv.\n"
            "  The sandbox venv isolates any pip installs from Ghost's environment.\n"
            "**Level 3 — Browser automation**: If scripts can't reach the data (login walls, JS-only "
            "content), use the browser tool to navigate, click, and extract interactively.\n"
            "**Level 4 — Combine approaches**: Chain tools together — e.g. browser to get a cookie, "
            "then Python script with that cookie to call an API.\n\n"
            "After you solve the task, if the solution would be useful again, submit it as a permanent "
            "Ghost tool:\n"
            "  `add_future_feature(title='Add <tool_name> tool', description='<what it does, "
            "the working script from /tmp/ghost_sandbox/run.py, and the pip deps>', "
            "priority='P1', source='user_request')`\n"
            "This queues it for the Evolution Runner to implement properly through the evolve loop "
            "and PR review system — keeping Ghost's codebase clean.\n\n"
            "## AVAILABLE TOOLS\n" + ", ".join(tool_names) + "\n\n"
            "## TOOL GUIDE\n"
            ...
            "- Install dependencies in `/tmp/ghost_sandbox/.venv` — NEVER into Ghost's own `.venv`.\n"
            "### Code Change Rules\n"
