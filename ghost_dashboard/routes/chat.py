...
def _process_message(session, daemon):
    ...
    try:
        ...
        chat_prompt_body = (
            ...
            "2. Tell the user the fix has been queued and will be implemented by the Evolution Runner.\n"
            "3. If you can work around the bug NOW (e.g. using shell_exec), do that too — "
            "but ALWAYS queue the fix. Never just work around it without fixing the root cause.\n\n"
            "## AVAILABLE TOOLS\n" + ", ".join(tool_names) + "\n\n"
            "## TOOL GUIDE\n"
